...
O comportamento ocorre na troca da filial de venda de **50 para 7**: a **filialNF** não é atualizada junto. O pedido inicia na filial **50** com **filialNF = 50** e, ao alterar a filial de venda para **7**, a **filialNF permanece 50**.  
Com isso, a política de **preço fixo** não é validada corretamente, porque a venda fica na filial **7**, mas a busca continua sendo feita com base na **região da filialNF 50**.

O comportamento é controlado pelo parâmetro **IGUALAR_FILIALNF_AO_ALTERAR_FILIAL**, que **não está cadastrado para todos os RCAs** desse cliente.

Foi validado também que **não haverá impacto para esse cliente**, porque ele **não usa filialNF nos cadastros dos clientes**.

**Ação recomendada**

1. Verificar no banco nuvem se o parâmetro **IGUALAR_FILIALNF_AO_ALTERAR_FILIAL** existe para os RCAs.
2. Ativar o parâmetro com valor **S**.
3. Desativar o parâmetro **FILIALNF_DEFINE_FILIAL_PEDIDO**.
4. Se houver necessidade de seleção manual da filialNF no pedido, habilitar para os RCAs a permissão **"Permitir escolha da filial de emissão da NF"**.
