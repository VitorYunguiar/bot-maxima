# Caso 1 - O vendedor destak.rca não consegue iniciar um pedido no cliente 6716
...
# Caso 2 - O cliente solicitou alterações no espelho do pedido: 1) Remoção do campo de impostos; 2) Remoção do "Impresso em" e...
...
# Caso 3 - O produto de código 56558 aparece na aba de produtos da base do RCA, mas não é encontrado ao fazer pedidos para os...
...
# Caso 4 - O cliente deseja que o preço unitário seja exibido na tela de negociação
...
# Caso 5 - Um pedido com um produto a R$47 foi cancelado
...
# Caso 6 - Ao inserir um produto de campanha, a quantidade inserida no campo sempre retorna para 1, impossibilitando a venda de...
...
# Caso 7 - Após parametrizar a exibição da lucratividade, o valor apresentado (aproximadamente 92,98%) não se altera...
...
# Caso 8 - Vários produtos da linha Red Bull (ex: 1537) não aparecem no maxPedido para o RCA disdal.2675 no cliente 55731
...
# Caso 9 - O cliente deseja que, se qualquer cliente de uma rede (principal ou secundário) estiver bloqueado, seja impedida a...
...
# Caso 10 - Um pedido bonificado (TV5) está travado na base do RCA LYNKZ.miguel (numped 256014561) e não é enviado para a nuvem...
...
# Caso 11 - Uma política de preço fixo para o produto 8153 é aplicada corretamente na filial 50, mas não é apresentada quando o RCA...
...
# Caso 12 - Vendedores não conseguem iniciar pedidos devido a um aviso de roteiros pendentes do dia anterior
...
# Caso 13 - Ao acessar as campanhas de desconto 2930 e 2931, a mensagem "não possui nenhum item disponível" é exibida
...
# Caso 14 - O cliente tem uma regra de comissão onde o RCA só ganha comissão se o desconto concedido for menor que 6%
...
# Caso 15 - O cliente deseja que pedidos bonificados sejam barrados no maxPedido caso o RCA não tenha saldo de conta corrente...
...
# Caso 16 - Ao tentar usar o plano de pagamento 230 com a cobrança 237, o maxPedido exibe erro informando que o plano não aceita...
...
# Caso 17 - O produto 99847 não aparece para o vendedor na filial 9 quando o cliente é o 232332 (com planos específicos), mas...
...
# Caso 18 - O cliente vai começar a usar campanhas de desconto e quer saber quais as alterações no JSON de integração do pedido...
...
# Caso 19 - O cliente deseja impedir que o RCA salve um pedido com valor acima do limite de crédito do cliente
...
# Caso 20 - O cliente quer substituir a informação de "Código de Fábrica" pela "Marca do produto" na listagem de produtos do...
...
# Caso 21 - Um pedido (NUMPED 3200118) foi rejeitado pela integradora devido a nível de venda, mas o link de pagamento do maxPag...
...
# Caso 22 - Ao tentar abrir ou editar um pedido específico (821977), o aplicativo exibe a mensagem: "O pedido
...
# Caso 23 - Os vendedores 1160 e 1161 não conseguem iniciar pedido em nenhum cliente
...
# Caso 24 - O cliente possui os parâmetros GPS_TRACKING_ENABLED = S e CONFIRMA_ATUALIZACAO_COORDENADA_CLIENTE = S, mas ao finalizar...
...
# Caso 25 - O analista pergunta qual a parametrização para bloquear que o vendedor envie um pedido bonificado se não tiver saldo de...
...
# Caso 26 - Cliente quer saber quais as alterações no JSON de integração quando um pedido é enviado com desconto escalonado
...
# Caso 27 - Um pedido aparece como "L" na timeline do aplicativo, mas já foi faturado
...
# Caso 28 - O alerta de economia de bateria continua aparecendo e impedindo o uso do maxPedido, mesmo com os parâmetros...
...
# Caso 29 - Ao duplicar um pedido que contém produtos com desconto e trocar o cliente, os descontos são perdidos, mesmo...
...
# Caso 30 - Um cliente reclama que um campo específico (hint de múltiplo) aparece para alguns produtos e para outros não
...
# Caso 31 - O produto 451047 não aparece ao fazer um pedido para o cliente 61845, mas aparece normalmente no card de produtos para...
...
# Caso 32 - O produto KIT PROMO SH+COND+CP TLL KD NUTRKD 480ML (CÓD: 299) não aparecia no aplicativo
...
# Caso 33 - O cliente quer usar vínculos de plano de pagamento por região (MXSPLPAGREGIAO)
...
# Caso 34 - Ao incluir qualquer produto para o cliente 14370 (RCA 19), o aplicativo exibe o erro: "Não foi possível carregar a...
...
# Caso 35 - O pedido 2300008379, feito na rotina 316 do Winthor, não aparece na timeline do vendedor jbatista.368
...
# Caso 36 - O cliente quer uma configuração que permita que o RCA confeccione o pedido para um cliente com títulos em atraso, mas...
...
# Caso 37 - Durante a homologação da V3, o cliente observou que, ao alterar o tipo de venda para "entrega futura" (TV7), a V2 exibe...
...
# Caso 38 - Ao iniciar um pedido no cliente 11528, filial 2, e inserir o produto 59544, o maxPedido exibe o alerta de erro ao...
...
# Caso 39 - Um RCA que também é supervisor tem seu CODUSUR alterado diretamente na Central de Configurações
...
# Caso 40 - Usuários do perfil 700 não conseguem visualizar o campo "REGIÃO" no card de produtos, enquanto usuários do perfil 100...
...
# Caso 41 - Ao duplicar o pedido 4376433 para clientes diferentes, os valores finais do pedido são diferentes, mesmo selecionando a...
...
# Caso 42 - O cliente gerou um boleto personalizado (para remover a hora da data), mas ele não aparece como opção para o RCA na...
...
# Caso 43 - Na aba "Objetivos" do maxPedido, ao buscar os valores do mês atual, a mensagem "Nenhum dado encontrado" é exibida...
...
# Caso 44 - Ao tentar acessar qualquer produto na filial 6, o aplicativo exibe a mensagem: "O PRODUTO SELECIONADO NÃO POSSUI DADOS...
...
# Caso 45 - Ao iniciar um pedido no cliente 1014627 (que usa vínculos na filial/região 10), a aplicação apresenta "inconsistência...
...
# Caso 46 - O cliente questiona se é possível, em um mesmo pedido normal (TV1), ter o mesmo item sendo vendido uma vez como normal...
...
# Caso 47 - O cliente usa o parâmetro EMITIR_NOTIFICACAO_EST_COMPLETA e recebe notificações de toda movimentação de estoque...
...
# Caso 48 - Ao negociar o produto 91432 na filial 2, a lucratividade apresentada no maxPedido é sempre negativa, independentemente...
...
# Caso 49 - Para o cliente 13801, a MXSHISTORICOPEDC mostra apenas um pedido (44027032) de setembro, enquanto a PCPEDC no banco...
...
# Caso 50 - Em um pedido com cobrança Cartão de Crédito (maxPag), o crédito disponível do cliente (R$ 241,55) não foi descontado do...
...
# Caso 51 - Clientes cadastrados para serem faturados na filial NF 2 estão tendo seus pedidos faturados pela filial NF 1 no Winthor
...
# Caso 52 - Uma restrição por plano de pagamento (código 1) foi criada para o plano 081, incluindo o produto 015227-3
...
# Caso 53 - Um título para o cliente 02830801 foi baixado no sistema, mas no aplicativo ele ainda consta como aberto
...
# Caso 54 - Após enviar uma solicitação de aumento de limite de crédito, ela não é mais exibida no aplicativo, nem mesmo após ser...
...
# Caso 55 - O cliente não consegue liberar um pedido na rotina 336 do Winthor
...
# Caso 56 - Quando um RCA sem saldo de conta corrente aplica um desconto, uma mensagem de alerta é exibida, mas ao clicar em "OK"...
...
# Caso 57 - A duplicata 61610 consta como aberta no aplicativo e no banco nuvem, mas já foi baixada no Winthor
...
# Caso 58 - Uma comissão progressiva foi cadastrada (4% para descontos de 0 a 5%)
...
# Caso 59 - Na versão 3.251.9, ao importar um orçamento que continha produtos com desconto para um pedido, os descontos não eram...
...
# Caso 60 - O valor de vendas exibido no resumo do maxPedido para o RCA 73 (filtrado por departamentos 2 e 160) é de R$ 4.835,31
...
# Caso 61 - O cliente solicita que o campo CODDESCONTO seja gravado na tabela PCORCAVENDAI (e PCORCAVENDAC) quando um orçamento é...
...
# Caso 62 - O cliente quer ocultar qualquer informação de estoque
...
# Caso 63 - O analista pergunta se é possível ocultar a informação do preço de venda (PVENDA) que é exibida para cada item na tela...
...
# Caso 64 - Um cliente foi cadastrado via maxPedido pelo RCA 93
...
# Caso 65 - Após atualizar o ambiente, os valores na dashboard inicial (gráfico de vendas) não são apresentados, embora o resumo de...
...
# Caso 66 - Cliente excluiu uma campanha de desconto (201179), mas ela continua aparecendo no maxPedido para o RCA
...
# Caso 67 - Analista pergunta qual tabela faz o vínculo dos grupos de clientes utilizados no campo `MXSPRECOPROM.CODGRUPOCLI` para...
...
# Caso 68 - Ao transformar um orçamento em pedido, a margem de lucratividade muda drasticamente
...
# Caso 69 - Cliente deseja que pedidos de brinde (TV5) sejam enviados para integração junto com o pedido principal (TV1), sem...
...
# Caso 70 - Cliente OERPs reporta que o roteiro de visitas não é gerado na tabela MXSCOMPROMISSOS, embora os registros existam na...
...
# Caso 71 - Há uma divergência no valor atendido (VLATEND) entre as tabelas MXSHISTORICOPEDC (nuvem) e PCPEDC (local) para o pedido...
...
# Caso 72 - O maxPedido apresenta preço divergente da rotina 316 para produtos com tributação SUFRAMA quando um desconto é aplicado
...
# Caso 73 - Pedidos já integrados (status 4) não atualizam o status na timeline do RCA, que usa sincronização automática e não pode...
...
# Caso 74 - Cliente possui uma política de desconto da rotina 561 (CREDITASOBREPOLITICA = N, BASECREDDEBRCA = S)
...
# Caso 75 - Campo 'Data Entrega' (DataPrevisaoFaturamento) de um relatório personalizado não é preenchido em alguns pedidos, embora...
...
# Caso 76 - Há uma divergência nos valores de histórico de compras para o RCA 62 e cliente 216
...
# Caso 77 - Títulos pagos não estão sendo baixados corretamente no aplicativo
...
# Caso 78 - Descontos em um pedido são tratados de forma diferente entre o maxPedido e a rotina 316 do Winthor
...
# Caso 79 - Os produtos 19679 e 13751 não são exibidos ao iniciar um pedido na filial 6, embora apareçam no card de produtos
...
# Caso 80 - Ao exportar a pesquisa de produtos com fotos na Central de Configurações, o arquivo Excel é limitado a 2001 registros...
...
# Caso 81 - Após atualizar para a versão 3.252.1, a seleção de região na aba de produtos desaparece, gerando um erro
...
# Caso 82 - Nenhum produto é exibido na base do RCA após sincronização, embora na base do zero os produtos apareçam normalmente
...
# Caso 83 - Ao clicar em 'Atualizar menu', o sistema exibe o erro: 'Não foi possível atualizar o resumo de vendas'
...
# Caso 84 - O produto 801797 não aparece devido à divergência nas tabelas de embalagem (MXSEMBALAGEM) e produto por filial...
...
# Caso 85 - Ao tentar abrir um produto no maxPedido, é exibida a mensagem de erro de tributação
...
# Caso 86 - A legenda de 'produto em promoção' continua sendo exibida para um produto, mesmo após a política de desconto/preço fixo...
...
# Caso 87 - O produto 2266 não é apresentado na aba 'Tabela'
...
# Caso 88 - Analista pergunta se existe uma forma de a APK deletar orçamentos antigos automaticamente após um período (ex: 30 dias)
...
# Caso 89 - O cliente configurou parâmetros para permitir que apenas a filial 5 possa iniciar orçamentos em clientes bloqueados...
...
# Caso 90 - Após uma atualização para corrigir um problema de timeline, os pedidos do cliente (que usa sincronização automática)...
...
# Caso 91 - Ao importar uma planilha com cerca de 7 mil clientes para uma campanha de brinde na Central, o sistema informa que 1.4...
...
# Caso 92 - O produto 1055 não aparece na base de nenhum vendedor, mas aparece na base do zero
...
# Caso 93 - Os registros dos RCAs 2543 e 2544 não foram encontrados na tabela MXSUSUARIOS na nuvem, indicando falha na replicação
...
# Caso 94 - Há uma grande divergência entre os títulos pagos e a previsão de comissão no relatório do Winthor (rotina 1248) e os...
...
# Caso 95 - Cliente precisa obrigar o RCA a seguir a sequência de visitas do roteiro do dia, não permitindo pular clientes
...
# Caso 96 - Uma solicitação de aumento de limite de crédito feita por um RCA não aparece no maxGestão nem na tabela...
...
# Caso 97 - Uma política de desconto cadastrada na Central para o cliente 37200 (produto 22164) não é listada no aplicativo, embora...
...
# Caso 98 - Analista pergunta se os parâmetros VERIFICAR_QTD_MAX_ITENS_PEDIDO e VERIFICAR_QTD_MAX_ITENS_PEDIDO_NRO funcionam por...
...
# Caso 99 - O campo de código de fábrica aparece cortado na metade na listagem de produtos para um RCA com um celular Galaxy A01...
...
# Caso 100 - Após alterar o horário de execução do replicador, o cliente começou a enfrentar locks no banco de dados local...
...
# Caso 101 - No relatório 8022 (Apuração de campanhas), o filtro 'CAMPANHA' não carrega os dados cadastrados no Winthor
...
# Caso 102 - As faixas de comissão enviadas pelo cliente na tabela MXSFAIXACOMISSAOUSUR não são exibidas para o vendedor ao...
...
# Caso 103 - Alterações nos vínculos de clientes com usuários (campos CODUSUR), feitas diretamente no banco ou na rotina 302, não...
...
# Caso 104 - Ao tentar cadastrar um novo cliente via APK, o campo e-mail é preenchido automaticamente pela busca de CNPJ, mas o...
...
# Caso 105 - Após uma carga de dados para a filial 12, a filial aparece na base do vendedor e os produtos são exibidos, mas a filial...
...
# Caso 106 - Mesmo com o RCA cumprindo todo o roteiro do dia anterior (15/10), o sistema bloqueou o início do roteiro no dia...
...
# Caso 107 - O produto 11 apresenta divergência de preço entre a MXSTABPR (nuvem) e a PCTABPR (local)
...
# Caso 108 - O produto 17477 não aparece na base dos vendedores, embora apareça na base do zero
...
# Caso 109 - O extrator do cliente (OERPs) está em loop, reiniciando constantemente, e o hangfire fica offline
...
# Caso 110 - Após uma carga de filial nova (filial 12), o vínculo do cliente 14380 com a nova filial, cadastrado na PCTABPRCLI...
...
# Caso 111 - As informações de última compra (data e quantidade vendida por mês) não são exibidas na aba 'Tabela' do pedido
...
# Caso 112 - Ticket duplicado (clonado) do GATE-191
...
# Caso 113 - Há uma divergência nos títulos para o cliente 13169
...
# Caso 114 - O RCA 9231 não cumpriu todo o roteiro do dia 16/10, mas o aplicativo não bloqueou seu acesso no dia 17/10, como...
...
# Caso 115 - Ao duplicar um pedido, o valor final fica diferente do original devido a uma mudança na porcentagem de desconto em...
...
# Caso 116 - O gráfico de metas não é gerado para o RCA 2444, mesmo com as metas cadastradas no banco nuvem
...
# Caso 117 - A meta de visitas criada na central não aparece no aplicativo
...
# Caso 118 - Ao cadastrar um cliente com um CEP específico (77063174), o maxPedido não reconhece o endereço e exibe erro, embora o...
...
# Caso 119 - Cliente opera com faixas de lucratividade (ex: até 15% salva, acima de 15,01% envia para autorização)
...
# Caso 120 - Vários produtos vendidos para o cliente 135838 em outubro não geraram registros na tabela MXSPRODUTPOS (positivação de...
...
# Caso 121 - Após uma carga nas tabelas de plano de pagamento (MXSCOBPLPAG, MXSPLPAG) para corrigir um erro, a correção funcionou ao...
...
# Caso 122 - O cliente gostaria que, ao marcar o checkbox 'Unidade Master' na tela de negociação, a quantidade fosse automaticamente...
...
# Caso 123 - Uma campanha de desconto progressivo (ex: comprar no mínimo 12 unidades de uma família) não está validando quando a...
...
# Caso 124 - Cliente utiliza um WMS de terceiros e precisa integrar os dados de validade ao maxPedido
...
# Caso 125 - Ao duplicar um pedido, o sistema não permite que o RCA altere a filial do novo pedido, mesmo mantendo o mesmo cliente
...
# Caso 126 - Para alguns clientes (ex: 9999), as filiais 2 e 3 não aparecem para seleção no cabeçalho do pedido, embora apareçam...
...
# Caso 127 - O cliente utiliza os parâmetros FORCAR_ATUALIZACAO_CADASTRO_CLIENTE e DIAS_ATUALIZACAO_CADASTRO_CLIENTE
...
# Caso 128 - Aparente duplicação de títulos na consulta do app para o mesmo cliente
...
# Caso 129 - Ao duplicar um pedido e tentar alterar o plano de pagamento para E08, mantendo o preço informado, o maxPedido exibe uma...
...
# Caso 130 - Pedidos (1570981101, 1570981103) constam como faturados na PCPEDC (banco local), mas na MXSHISTORICOPEDC (nuvem) estão...
...
# Caso 131 - A APK está permitindo que um pedido de bonificação (TV5) seja vinculado a um pedido do mês 7 que já foi faturado
...
# Caso 132 - Alguns produtos, como o 506153, não aparecem no maxPedido para o RCA 110, indicando uma divergência entre a base do RCA...
...
# Caso 133 - Cliente utiliza vínculos na PCTABPRCLI, vinculando clientes apenas à filial 1
...
# Caso 134 - No relatório 'Visitas Previstas x Realizadas' do maxGestão, apenas um representante aparece no filtro, enquanto...
...
# Caso 135 - O cliente 18325 tem limite de crédito disponível, mas ao tentar fazer um pedido com o plano de pagamento '5 - 28 dias'...
...
# Caso 136 - Cadastros de clientes feitos pelo aplicativo estão duplicando na rotina 302 do Winthor
...
# Caso 137 - As metas mensais cadastradas pela rotina 3305 não estão sendo apresentadas no aplicativo para nenhum RCA
...
# Caso 138 - Na filial 50, o desconto de 3% do plano de pagamento 79 não está sendo aplicado a um produto com preço fixo, embora...
...
# Caso 139 - Há uma divergência de valores entre o maxPedido e o maxPag para o pedido 801278
...
# Caso 140 - O plano de pagamento 2 (PLPAG 2) está com o campo PERTCFIM divergente entre o banco local (-3%) e o banco nuvem (0%)
...
# Caso 141 - Problema recorrente no cliente Muffato: pedidos bonificados (TV5) demoram muito para serem enviados para a nuvem após o...
...
# Caso 142 - Cliente reporta novamente que um RCA cumpriu todo o roteiro do dia anterior, mas o sistema bloqueou o acesso no dia...
...
# Caso 143 - O produto 92656 não aparece para o RCA 956, embora apareça para outros e as tabelas principais pareçam ok
...
# Caso 144 - Durante a migração da V2 para a V3, uma campanha de preço fixo cadastrada na Central para o produto 14377 (valor 24.49)...
...
# Caso 145 - O campo 'Observação de entrega' (OBSENTREGA) na MXSHISTORICOPEDC não está sendo exibido no aplicativo para o pedido...
...
# Caso 146 - Um relatório personalizado, cuja alteração foi combinada com um antigo desenvolvedor, ficou incompleto
...
# Caso 147 - O produto 10742 possui apenas uma embalagem ativa para o força de vendas, mas na APK são apresentadas duas embalagens...
...
# Caso 148 - Alguns produtos, como o 59, possuem uma imagem cadastrada, mas ao acessá-la, o que se vê é uma imagem padrão de...
...
# Caso 149 - A seleção múltipla de produtos está calculando a quantidade final de forma incorreta
...
# Caso 150 - O produto 46120023 não aparece na base dos RCAs 277, 324 e 372, embora apareça na base do zero
...
# Caso 151 - A sincronização automática foi liberada para o cliente, mas o status dos pedidos não está atualizando, mesmo com o...
...
# Caso 152 - Na base do RCA, o preço de alguns produtos está sendo exibido como o valor da embalagem, e não o valor unitário
...
# Caso 153 - As informações de endereço (comercial e de entrega) de um cliente (119619) estão trocadas no aplicativo em comparação...
...
# Caso 154 - Títulos de 2022 para o cliente 4861 estão em aberto na ERP_MXSPREST (nuvem), mas não existem mais na PCPREST (local)
...
# Caso 155 - Mesmo com os parâmetros de bloqueio de rota pendente configurados (JUSTIFICAR_ROTEIRO_ANTERIOR=S...
...
# Caso 156 - Cliente utiliza preço fixo (MXSPRECOPROM) e planos de pagamento com desconto (PERTXFIM)
...
# Caso 157 - Pedidos cancelados no ERP (ex: 5100121233) não têm seu status atualizado no maxPedido, permanecendo como liberado
...
# Caso 158 - Informações de carregamento de pedidos feitos como 'balcão reserva' não estão sendo exibidas nos 'Dados do ERP' no...
...
# Caso 159 - Ao tentar usar a campanha de desconto 7709, que contém o item 164889, o sistema retorna erro informando que o item não...
...
# Caso 160 - Um RCA específico (843) do perfil 700 está tendo seu pedido com cobrança de cartão de crédito rejeitado, com a mensagem...
...
# Caso 161 - Há múltiplos problemas com uma cesta de natal (produto 251425): preço na listagem difere do preço na negociação, ao...
...
# Caso 162 - Para o cliente 13485, ao iniciar um pedido de bonificação, o item 183 carrega um múltiplo de 12 unidades
...
# Caso 163 - O analista questiona de onde vem o valor de IPI de alguns produtos, pois na MXSTABPR o campo VLIPI está 0 em todas as...
...
# Caso 164 - Os títulos não são apresentados corretamente na consulta do aplicativo
...
# Caso 165 - Ao gerar um boleto, o código de barras apresenta problemas de leitura em aplicativos de bancos
...
# Caso 166 - O banco do cliente 'Reposit' está com alto processamento devido a vários processos do MAXSOLUCOES

**Contexto**

O banco do cliente 'Reposit' está com alto processamento devido a vários processos do MAXSOLUCOES. Relatório AWR
...
# Caso 167 - Na tabela ERP_MXSPREST, títulos abertos referentes ao período 30/05 a 09/06 divergem do banco local: banco nuvem tem...
...
# Caso 168 - Há divergência de registros entre PCTABPR e MXSTABPR
...
# Caso 169 - O cliente envia as tabelas ERP_MXSNFSAID e ERP_MXSMOV para gerar dados na MXSQTDEPRODVENDA, mas a tabela não é gerada
...
# Caso 170 - O preço de uma cesta no maxPedido está divergente da rotina 316
...
# Caso 171 - Os status dos pedidos não atualizam para os RCAs e ficam divergentes do banco nuvem
...
# Caso 172 - Preço de cesta no maxPedido divergente da rotina 316
...
# Caso 173 - Divergência entre PCPREST e ERP_MXSPREST: na PCPREST, DTPAG e VPAGO estão preenchidos (pago), mas na ERP_MXSPREST e...
...
# Caso 174 - Produto não aparece mesmo tendo precificação, estoque, acesso a fornecedor, departamento e seção, e sem restrições de...
...
# Caso 175 - Os dados do resumo de vendas da filial 6 não são apresentados na aba de departamento (zerado)
...
# Caso 176 - Pedidos bonificados gerados automaticamente não são apresentados no histórico de pedidos
...
# Caso 177 - Extrator Tcloud offline e banco Winthor inacessível
...
# Caso 178 - Clientes não aparecem na base do RCA (LDN.34)
...
# Caso 179 - Algumas fotos de produtos não são processadas na job de fotos do ponto de montagem da NORDIL
...
# Caso 180 - Ao consultar produtos na aba 'Produtos', a APK não retorna registros na versão ponta, mas na versão 3.257.0 funciona
...
# Caso 181 - Pedido de R$16.000 (numped 19471392) foi pago via PIX, mas não chegou ao ERP
...
# Caso 182 - Não há permissão na central de configurações para reenviar pedido bloqueado
...
# Caso 183 - Ao justificar visita em cliente fora de rota, mesmo salvando a justificativa, o sistema não autoriza o check out
...
# Caso 184 - Produto não aparece para o RCA, embora haja preço na MXSTABPR para as filiais que o RCA tem acesso, estoque e...
...
# Caso 185 - Produto 1505.0 não pode ser inserido em pedido na filial 1, retornando erro de tributação
...
# Caso 186 - Planos de pagamento para o cliente 10201: na PCPLPAGCLI existem 4 registros, mas na MXSPLPAGCLI apenas 2, causando...
...
# Caso 187 - Após queda de energia, o menu do maxPedido parou de atualizar
...
# Caso 188 - Produtos similares exibidos no APK são diferentes dos cadastrados na MXSPRODSIMIL (cliente GBM)
...
# Caso 189 - Na aba 'Movimentação' do pedido, algumas informações (data de montagem, faturamento, conferente) não são exibidas...
...
As informações vêm da MXSHISTORICOPEDC. O script que carrega a movimentação foi fornecido

# Caso 190 - É possível configurar o sistema para que, ao ultrapassar a quantidade de pedidos fora de rota (QTD_MAX_PED_FORA_ROTA)...
...
# Caso 191 - Plano de pagamento 045, com valor mínimo de pedido R$300, não bloqueou pedido de R$60
...
# Caso 192 - Status de pedidos não atualizam pela sincronização automática
...
# Caso 193 - Resumo de vendas não atualiza para alguns RCAs (14565, 16024, 98229), mesmo com portas 9000/9002 liberadas
...
# Caso 194 - Quantidade de clientes positivados no app é 15, mas na MXSHISTORICOPEDC há 61 registros
...
# Caso 195 - Cadastro de cliente (CPF 738.627.703-20) pelo aplicativo não integra com Winthor, retornando crítica de código cliente...
...
# Caso 196 - Ao iniciar pedido para o cliente 2922, mensagem de inconsistência: nenhum plano de pagamento carregado
...
# Caso 197 - Valores de frete cadastrados na MXSCLIENT (VLFRETE=45, VLMAXCOBFRETE=800) não são aplicados no pedido
...
# Caso 198 - Registros da PCTABPRCLI para a filial 11 não desceram para a nuvem; apenas um registro consta na MXSTABPRCLI, enquanto...
...
# Caso 199 - Na MXSHISTORICOPEDI do pedido 517261, PTABELA e PVENDA são iguais, mas na ERP_MXSLOGRCA foi adicionado valor de 220...
...
# Caso 200 - Após migrar da V2 para V3, ao tentar inserir o produto 12111.0 no cliente 201864, ocorre erro de tributação
...
# Caso 201 - Permissão de acesso a cobrança já foi marcada na central, mas na base do RCA não atualiza, impedindo iniciar pedido
...
# Caso 202 - No pedido 183006815, o percentual de juros do plano de pagamento 22 deveria ser 3% (PERTX), mas o aplicativo aplicou...
...
# Caso 203 - Fornecedor 701 e seus produtos não são exibidos na APK
...
# Caso 204 - API de cancelamento do cliente Tcloud não funciona, mesmo com todas as informações corretas
...
# Caso 205 - Alerta de preço mínimo só aparece quando o preço é zero
...
# Caso 206 - Pedidos de origem telemarketing (ORIGEMPED = T) não são enviados para a MXSHISTORICOPEDC, mesmo com...
...
# Caso 207 - Cliente deseja que clientes bloqueados possam fazer pedidos à vista
...
# Caso 208 - Título duplicata 186134 está pago no banco local, mas na nuvem DTPAG e VPAGO estão vazios, aparecendo como devido
...
# Caso 209 - Pedidos estão sumindo do menu de pedidos para alguns RCAs, aparecendo apenas no histórico
...
# Caso 210 - Produto 96777 não aparece no maxPedido
...
# Caso 211 - Pedidos de clientes (ex.: 5523) estão sendo gerados na filial 1, mas deveriam ser apenas na filial em que são negociados
...
# Caso 212 - Campos 'Posição financeira', 'Contatos', 'Referências comerciais' não desabilitam na central de configurações, mesmo...
...
# Caso 213 - Valores do resumo de vendas não batem com o ERP para o vendedor Cláudio (RCA 7348) no mês de dezembro
...
# Caso 214 - Duplicata 669805 foi baixada no Winthor, mas no banco nuvem ainda consta pendente (DTPAG e VPAGO vazios)
...
# Caso 215 - Conta corrente do vendedor não apresenta dados
...
# Caso 216 - Pedidos abaixo de R$200 não são salvos devido a mensagem de valor mínimo, mas a cobrança BKBB está configurada com...
...
# Caso 217 - Para clientes da 'Rede Mais', o RCA está negociando produtos com preço diferente do que aparece na base do zero
...
# Caso 218 - Pedidos do RCA (3201016992, 3201016993, 3201016994) não aparecem no app, embora constem na MXSINTEGRACAOPEDIDO
...
# Caso 219 - Em orçamentos, é possível aplicar qualquer desconto (até 100%), sem bloqueio
...
# Caso 220 - Faixas de comissão cadastradas na MXSCOMISSAOPLPAG não são exibidas para o plano de pagamento 2, mas são exibidas para...
...
# Caso 221 - Valores exibidos no maxGestão para vendas transmitidas divergem da rotina 146 do Winthor
...
# Caso 222 - Ao fazer pedido com o item 29825, a integradora rejeita pois o CODAUXILIAR 10061 não existe na PCEMBALAGEM, apenas na...
...
# Caso 223 - RCAs estão conseguindo enviar pedidos com valor abaixo da restrição de R$300
...
# Caso 224 - O cliente reporta que um vendedor específico tem 71 dias sem visitar um cliente, mesmo tendo feito um pedido...
...
# Caso 225 - O cliente deseja ocultar a aba de títulos pendentes no cadastro do cliente no MaxPedido
...
# Caso 226 - Uma indenização (NUMINDENIZACAO 80254) foi criada no MaxPedido, retornou crítica de sucesso, mas não aparece nas...
...
# Caso 227 - O pedido 528221828 foi pago via MaxPag e processado no ERP, mas no portal MaxPag ainda consta como 'Pré-Aprovado'
...
# Caso 228 - O resumo de vendas não atualiza para dezembro de 2024, exibindo 'Sem dados', embora o Hangfire esteja funcionando
...
# Caso 229 - Ao tentar inserir itens no cliente 18748, todos os produtos retornam erro de tributação, mesmo após verificação das...
...
# Caso 230 - O status de pedidos não atualiza automaticamente para múltiplos usuários, mesmo após correções anteriores
...
# Caso 231 - O produto 29476.0 não aparece para o RCA 132 em qualquer cliente, embora exista na base e tenha todas as permissões...
...
# Caso 232 - O plano de pagamento 3, liberado para o vendedor na central de configurações, não é exibido para o cliente 850, mas...
...
# Caso 233 - O produto 001934-7 recebe um acréscimo de 10 centavos ao alterar o plano de pagamento, enquanto outros produtos não
...
# Caso 234 - Diversos produtos com histórico de venda nos últimos 3 meses não aparecem na sugestão de venda para o cliente 8701
...
# Caso 235 - Em pedidos salvos e bloqueados, a autorização de preço (rotina 301) não é carregada, mesmo após sincronização
...
# Caso 236 - Os produtos similares exibidos no MaxPedido para o produto 9259 são diferentes dos cadastrados na MXSPRODSIMIL
...
# Caso 237 - Pedidos estão sendo enviados duplicados para a MXSINTEGRACAOPEDIDO, enquanto na base do RCA consta apenas um
...
# Caso 238 - O produto 1505.0 não pode ser inserido em pedidos na filial 1 devido a erro de tributação
...
# Caso 239 - Poucos planos de pagamento aparecem para o cliente 10201
...
# Caso 240 - Após uma queda de energia, o menu do MaxPedido parou de atualizar
...
# Caso 241 - O cliente quer que as informações de movimentação do pedido (data de montagem, faturamento, etc.) no MaxPedido...
...
As informações da aba 'Movimentação' do pedido são provenientes da tabela MXSHISTORICOPEDC. O script de consulta utilizado pelo MaxPedido foi fornecido para referência

# Caso 242 - O cliente deseja que, ao ultrapassar a quantidade de pedidos fora de rota definida pelo parâmetro...
...
# Caso 243 - O plano de pagamento 045 possui valor mínimo de pedido de R$300, mas o sistema permitiu a criação de um pedido de R$60
...
# Caso 244 - O resumo de vendas não atualiza para alguns RCAs, embora as portas 9000 e 9002 estejam liberadas e funcione para outros
...
# Caso 245 - O filtro de comissão diferenciada no MaxPedido não está funcionando como o cliente espera
...
# Caso 246 - O resumo de vendas não atualiza para os usuários 14565, 16024 e 98229, embora as portas estejam liberadas
...
# Caso 247 - O campo de desconto no boleto gerado pelo MaxPedido está em branco, mesmo com informações na tabela
...
# Caso 248 - A quantidade de clientes positivados mostrada no MaxPedido (15) é diferente da encontrada na MXSHISTORICOPEDC (61)
...
# Caso 249 - Um cadastro de cliente (CPF 738.627.703-20) feito pelo aplicativo não integra com o Winthor, retornando crítica de...
...
# Caso 250 - Ao tentar iniciar um pedido para o cliente 2922, o sistema informa inconsistência nos dados e que nenhum plano de...
...
# Caso 251 - A API de cancelamento de pedidos não funciona para o cliente T-Cloud, mesmo com as informações de IP, usuário e senha...
...
O problema era a configuração do IP no extrator do cliente. Foi necessário utilizar o IP sem a porta ( porque, no ambiente do cliente, a porta do WTA não está liberada para acesso externo. Após ajuste e reinicialização do extrator, a API de cancelamento passou a funcionar

# Caso 252 - O sistema não está bloqueando descontos abaixo do preço mínimo, a menos que o preço seja zero
...
# Caso 253 - Pedidos de origem telemarketing (ORIGEMPED = T) não estão sendo enviados para a MXSHISTORICOPEDC, mesmo após habilitar...
...
# Caso 254 - O cliente quer que clientes bloqueados por inadimplência possam fazer pedidos à vista, mas o sistema permite pedidos...
...
# Caso 255 - Um título (duplicata 186134) aparece como em aberto no MaxPedido, mas está pago no Winthor
...
# Caso 256 - O pedido 3802108600 não aparece na timeline de pedidos do RCA, embora exista na base de dados e não tenha sido deletado
...
# Caso 257 - O produto 96777 não aparece no MaxPedido
...
# Caso 258 - O cliente deseja restringir que determinados clientes façam pedidos em uma filial específica (ex: cliente 5523 não deve...
...
# Caso 259 - O resumo de vendas do vendedor Claudio não bate com o valor do ERP para dezembro
...
# Caso 260 - Uma duplicata (669805) está baixada no Winthor, mas permanece pendente no banco nuvem
...
# Caso 261 - Ao tentar realizar pedidos com o item 29825, a integradora rejeita, informando que o CODAUXILIAR 10061 não existe na...
...
# Caso 262 - Alguns RCAs estão conseguindo enviar pedidos com valores abaixo do limite de R$300 definido em uma restrição
...
# Caso 263 - Os descontos não estão sendo aplicados aos itens no pedido, mesmo após cadastro das políticas
...
# Caso 264 - Pedidos estão sendo gravados na PCPEDC, mas não aparecem na MXSHISTORICOPEDC
...
# Caso 265 - A data de vencimento dos produtos não aparece no aplicativo na opção 'dados adicionais'
...
# Caso 266 - Pedidos do tipo balcão reserva não aparecem no histórico de pedidos (aba Consultas), mas aparecem na timeline
...
# Caso 267 - O cliente deseja entender o fluxo de comissão no MaxPedido, incluindo endpoints e tabelas envolvidas
...
# Caso 268 - Um pedido foi confeccionado fora de rota/raio sem solicitar autorização de desbloqueio, mesmo com as permissões de...
...
# Caso 269 - Duas políticas de desconto foram excluídas no ERP, mas ainda aparecem no MaxPedido
...
# Caso 270 - O cliente questiona por que todos os clientes estão mostrando o plano de pagamento de 7 dias no cabeçalho do pedido, em...
...
# Caso 271 - Pedidos com origem telemarketing não estão sendo enviados para as tabelas MXSHISTORICOPEDC e MXSHISTORICOPEDI
...
# Caso 272 - O produto 701 apresenta o valor unitário dividido (ex: valor 84,99, valor UN 44,50)
...
# Caso 273 - O produto 1580 não aparece no MaxPedido
...
# Caso 274 - O relatório de metas não é gerado para alguns RCAs, embora a meta conste no banco nuvem e os dias úteis estejam...
...
# Caso 275 - O status de pedidos não atualiza na sincronização automática, mesmo com os pedidos já integrados no ERP
...
# Caso 276 - A campanha 42 está incluindo o produto 4244, que não deveria fazer parte dela
...
# Caso 277 - O campo 'PrecoTabelaSemImpostos' no JSON do pedido está sendo enviado com valor multiplicado (4968) na versão 3.268.3...
...
# Caso 278 - Ao trocar o plano de pagamento para 'CREDITO A VISTA', o aplicativo alega desconto máximo excedido, mesmo sem descontos...
...
# Caso 279 - Todos os produtos apresentam erro de tributação ao tentar fazer pedidos
...
# Caso 280 - A conta corrente do vendedor Arthur apresenta valores como se houvesse acréscimos aplicados ao Flex, mas o cliente...
...
# Caso 281 - O cliente quer que a meta diária seja apresentada no resumo de vendas, mas atualmente só aparece a meta mensal de 35000
...
# Caso 282 - Os registros na MXSPRODUTPOS estão sendo gerados com CODOPERACAO = 2 (deletados), mesmo para registros recentes
...
# Caso 283 - O relatório 8018 no maxGestão não lista as filiais no filtro, embora a listagem funcione no Winthor
...
# Caso 284 - O campo PrecoOriginal no JSON está sendo enviado com o valor total da embalagem (PTABELA * fator), em vez do preço de...
...
# Caso 285 - Pedidos do tipo balcão reserva não são enviados para a MXSHISTORICOPEDC, mesmo com o parâmetro...
...
# Caso 286 - Uma vendedora (RCA 5) só vê os títulos dela, mas precisa ver os títulos de outros RCAs (81 e 85) que compartilham a...
...
# Caso 287 - A posição (status) dos pedidos não atualiza na timeline de múltiplos RCAs, mesmo com a sincronização automática
...
# Caso 288 - Há uma divergência de comportamento entre a base do zero e a base de um RCA: produtos não aparecem para o vendedor...
...
# Caso 289 - Ao tentar iniciar um pedido para o cliente 5107, o sistema retorna uma mensagem informando que nenhum plano de...
...
# Caso 290 - O produto 15294 apresenta uma previsão de recebimento no MaxPedido, mas o analista verificou que não há previsões...
...
# Caso 291 - O relatório 8020 apresenta erro ao ser gerado no maxPedido, embora funcione no Winthor
...
# Caso 292 - Ao pesquisar um produto pelos últimos 4 dígitos do código de barras (1059), o aplicativo retorna múltiplos produtos, e...
...
# Caso 293 - Durante a sincronização, uma mensagem de conclusão aparece no início, e ao clicar em OK, o sistema entra em loop...
...
# Caso 294 - Acréscimos estão sendo enviados do ERP para a conta corrente do RCA sem que o vendedor tenha movimentado a conta (Flex)
...
# Caso 295 - Cliente solicita a limpeza dos objetos não utilizados do schema do Pedido de Vendas em seu banco local
...
# Caso 296 - Uma política de desconto automática (10959), vinculada à região 1, é aplicada corretamente quando a filial 1 é...
...
# Caso 297 - O cliente utiliza uma view personalizada do WMS (SR_VALIDADE) que alimenta a tabela MXSVALIDADEWMS no MaxPedido
...
# Caso 298 - Cadastros de clientes realizados pelo aplicativo não estão gravando campos de endereço (comercial e cobrança) na tabela...
...
# Caso 299 - Para o produto 4557, a tributação não é carregada quando a FILIALNF 2 é selecionada, embora exista vínculo na...
...
# Caso 300 - Os dados de conta corrente não são exibidos na versão 4 do aplicativo, mesmo com os parâmetros DEFINE_CC_MENU e outros...
...
# Caso 301 - Vários produtos não aparecem na base do RCA (ex: 5320), mas aparecem na base do zero
...
# Caso 302 - O gráfico 'Clientes positivados' permanece zerado no aplicativo, mesmo com pedidos na base do RCA e registros nas...
...
# Caso 303 - Ao tentar duplicar um pedido faturado (posição F), o aplicativo retorna uma mensagem de erro informando que não foi...
...
# Caso 304 - Ao tentar visualizar o histórico de um pedido na base do RCA, o aplicativo solicita que seja feita uma sincronização...
...
# Caso 305 - Os gráficos de valores vendidos no mês não são gerados na versão 4 do MaxPedido, embora o resumo de vendas apresente os...
...
# Caso 306 - Uma política de preço fixo (252487) não está sendo aplicada no MaxPedido para o item 2237, nas condições especificadas...
...
# Caso 307 - Os itens do carregamento de pronta entrega 152743 não aparecem para o RCA
...
# Caso 308 - Ao tentar inserir o item 3614 em um pedido, ocorre um erro
...
# Caso 309 - O aplicativo está apresentando consumo de bateria excessivo (82,1%) em segundo plano, mesmo sem uso ativo
...
# Caso 310 - Após a troca do IP na stack do extrator (de 192.168.0.9:8180 para 192.168.9.25:8180), a API de cancelamento de pedidos...
...
O problema era a falta de uma barra ('/') no final da URL na variável de ambiente da stack do extrator. A correção foi aplicar a URL correta: ' Após o ajuste e novo deploy da stack, a API de cancelamento passou a funcionar

# Caso 311 - O vendedor (mix.302) recebe um alerta de roteiro pendente do dia anterior ao tentar iniciar um pedido, mesmo tendo...
...
# Caso 312 - O cliente relata que processos da Máxima estariam causando locks no banco local e travando o Winthor
...
# Caso 313 - Uma política de preço fixo (252487) não está sendo aplicada no MaxPedido (mesmo contexto do GATE-757)
...
# Caso 314 - As solicitações de limite de crédito do RCA 2 não estão chegando no maxGestão para aprovação
...
# Caso 315 - Ao importar uma planilha para editar o mix ideal na central de configurações, o sistema retorna erro informando que...
...
# Caso 316 - Há uma divergência de preço para o produto 21697 na região 3: o preço no banco local (PCTABPR) é R$ 11,39, enquanto no...
...
# Caso 317 - Roteiro pendente do dia anterior (19/02) está sendo exibido incorretamente para o RCA mix.120, com mais de 100...
...
# Caso 318 - Há divergência de registros de títulos entre a tabela PCPREST (banco local) e a ERP_MXSPREST (banco nuvem), causando a...
...
# Caso 319 - O cliente utiliza uma view personalizada do WMS e há divergência nas validades dos produtos entre o banco local e o...
...
# Caso 320 - Há divergência de registros de políticas de preço fixo entre a PCPRECOPROM (banco local) e a MXSPRECOPROM (banco nuvem)
...
# Caso 321 - Pedidos estão aparecendo duplicados na timeline, um com status 'Liberado' e outro com 'Faturado', embora no banco...
...
# Caso 322 - Vendedores estão conseguindo bater o ponto fora do horário de tolerância configurado na central
...
# Caso 323 - O valor exibido na tela inicial (venda transmitida) está divergente do valor apresentado no resumo de vendas e na...
...
# Caso 324 - Mesmo com os parâmetros HABILITA_PED_CLI_NAO_SINC e HABILITA_PED_CLI_RECEM_CADASTRADO ativos e o campo CODFUNCULTALTER...
...
# Caso 325 - Ao tentar liberar uma versão do MaxPedido para um RCA pela rotina 304 da central de configurações, o sistema retorna o...
...
# Caso 326 - Duplicidade de pedidos na timeline (mesmo contexto dos GATE-820 e 838)
...
# Caso 327 - Um supervisor não consegue voltar para sua própria base no maxPedido após trocar para a base de um RCA
...
# Caso 328 - O cliente questiona por que na rotina 3330 a filial retira do cabeçalho é 2, enquanto a filial retira do produto é 5
...
# Caso 329 - Um pedido de balcão reserva não gerava nota e boleto
...
# Caso 330 - Clientes de um RCA não aparecem no maxPedido, mesmo após transferência de carteira na rotina 328 e carga interna
...
# Caso 331 - O cliente tem regiões vinculadas ao cliente pela MXSCLIENTREGIAO, mas ao inserir novas regiões, as anteriores são...
...
# Caso 332 - Um pedido com erro de envio ficou na posição 'pendente do ERP' na timeline, embora não tenha sido enviado
...
# Caso 333 - O cliente deseja que não seja possível vincular uma bonificação a um pedido de filial diferente
...
# Caso 334 - Para o cliente 14951, a política de desconto 657146 (13%) não é aplicada, apenas a 645376 (9,68%)
...
# Caso 335 - Um RCA não consegue iniciar pedido devido à mensagem de inconsistência de dados por falta de permissão a plano de...
...
# Caso 336 - Um RCA não vê algumas faixas de comissão na negociação, embora elas existam na mxscomissaousur
...
# Caso 337 - Os itens de um pedido cancelado não aparecem no histórico de pedidos, embora haja registros na MXSHISTORICOPEDI e os...
...
# Caso 338 - O cliente quer que ao entrar no aplicativo seja exigida a jornada de trabalho, mas atualmente só bloqueia ao iniciar um...
...
# Caso 339 - O limite de crédito disponível no maxPedido (R$ 961,03) diverge da rotina 1203 do Winthor (R$ 175,68)
...
# Caso 340 - Na versão 4.011.3, a lucratividade não aparece na aba de itens, enquanto na versão 3.270.4 aparecia
...
# Caso 341 - O ícone de promoção continua aparecendo para um produto mesmo após o fim da promoção
...
# Caso 342 - O parâmetro DEFINE_FILIAL_RETIRA_PADRAO está configurado como 5, mas o pedido sempre inicia na filial 2
...
# Caso 343 - O resumo de vendas apresenta valores divergentes: venda transmitida R$ 240.194,22, venda faturada R$ 241.379,90 e valor...
...
# Caso 344 - O cliente quer saber se é possível mencionar um cliente normal em um orçamento feito para o consumidor final
...
# Caso 345 - Um RCA não consegue consultar produtos na filial 2 porque nenhum item é retornado
...
# Caso 346 - Ao tentar cancelar um pedido, o sistema apresenta erro informando falha na API do Winthor
...
# Caso 347 - Ao selecionar uma filial de retirada com estoque, o sistema impede a inclusão do item, informando falta de estoque
...
# Caso 348 - O cliente habilitou o parâmetro HABILITA_MAPA_OPORTUNIDADE, mas a aba para preenchimento de CNAEs não aparece no perfil...
...
# Caso 349 - Um pedido não foi enviado para o maxPag, ficando apenas no ERP
...
# Caso 350 - O cliente tem dúvidas sobre o funcionamento da conta corrente: se é estornada ao cancelar um pedido e se ocorre...
...
# Caso 351 - O cliente não consegue visualizar o produto 30341 e suas embalagens
...
# Caso 352 - Pedidos realizados com pagamento TINO não são enviados ao ERP, ficando com a crítica 'Link maxPayment gerado com...
...
# Caso 353 - Ao iniciar qualquer pedido, o sistema apresenta mensagem de inconsistência por falta de permissão a plano de...
...
# Caso 354 - Ao alterar o tipo de venda para bonificação, o sistema mostra mensagem de que nenhuma cobrança pode ser carregada...
...
# Caso 355 - O produto 14929 não exibe a opção de solicitação de preço quando se aplica um desconto, embora em outros produtos...
...
# Caso 356 - O produto 3777, com estoque de 405 unidades, apresentou erro de falta de estoque ao ser inserido em pedido
...
# Caso 357 - O cliente quer que a quantidade mínima do produto 5718 seja 0,8, mas o maxPedido fixa em 1,0 mesmo após alterar QTUNIT...
...
# Caso 358 - O cliente não consegue fazer uma venda normal e adicionar um item bonificado
...
# Caso 359 - Produto sendo integrado com valor divergente: na nuvem consta R$ 8,99, mas no ERP chega R$ 71,92
...
# Caso 360 - O cliente deseja a criação da view de lotes de validade da WMS
...
# Caso 361 - Um RCA tem roteiro de visitas sem justificar, mas não consegue justificar o cliente
...
# Caso 362 - Um pedido foi autorizado no maxGestão, mas o RCA recebeu retorno de que o produto foi cortado por preço mínimo
...
# Caso 363 - Os itens do pedido 74793 não aparecem no histórico, embora constem na pcpedi
...
# Caso 364 - Ao compartilhar um espelho de pedido, o sistema apresenta alerta de falta de permissão à filial, mesmo com permissões...
...
# Caso 365 - Códigos de clientes cadastrados pelo aplicativo estão gerando números fictícios que já pertencem a outros clientes da...
...
# Caso 366 - Produtos novos (códigos 4022.0, 4023.0, 4024.0) não aparecem no maxPedido, embora estejam ativos para venda pelo Proton
...
# Caso 367 - Um RCA vinculado a duas campanhas de desconto escalonado (códigos 105 e 801) não vê as campanhas no aplicativo, embora...
...
# Caso 368 - Um pedido cancelado (nº 74793) não aparece na MXSQTDEPRODVENDA, embora haja histórico de vendas
...
# Caso 369 - O saldo da conta corrente de alguns RCAs ficou negativo, mesmo com parâmetros que deveriam impedir
...
# Caso 370 - Ao imprimir um boleto, aparece uma numeração abaixo do código de barras que não corresponde à linha digitável
...
# Caso 371 - O limite de crédito disponível no maxPedido difere do Winthor (R$ -1.036,64 vs R$ 1.000)
...
# Caso 372 - O RCA 914 não tem acesso a fornecedores PEPSICO quando acessa a base de um RCA exclusivo, embora tenha as mesmas...
...
# Caso 373 - O produto 34469 não aparece na negociação, embora esteja na base do RCA
...
# Caso 374 - O produto 1532 não aparece na base do RCA, mas aparece na base do zero
...
# Caso 375 - O cliente quer que orçamentos com mais de 3 dias não possam ser importados para pedido
...
# Caso 376 - O campo 'preço de venda sem impostos' aparece no Winthor, mas não no maxPedido
...
# Caso 377 - Ao tentar alterar a filial retira de um produto, apenas a filial 2 é exibida, quando deveriam ser 2 e 3
...
# Caso 378 - Ao duplicar um orçamento, o sistema exibe a mensagem 'Campanha excedeu valor máximo de descontos no período'
...
# Caso 379 - Um RCA não apresenta produtos para o cliente 9330, embora haja precificação na região 10 e acesso à região
