...
**Descrição**: O comportamento de venda para cliente bloqueado depende de parâmetros do maxPedido e, para clientes Winthor, também do parâmetro **CON_ACEITAVENDABLOQ** no ERP.

**Resumo operacional**: cliente bloqueado continua visível com ícone de cadeado. Se a configuração bloquear venda, o representante recebe aviso e pode salvar como orçamento quando essa opção estiver permitida.

**Fonte canônica**: manter a matriz completa de parâmetros e valores em `05-PEDIDOS-E-VENDAS.md`, seção **Cliente Bloqueado**. Para criação de parâmetro ausente, consulte a seção **1.11 Como criar parâmetros** neste documento.
...
**Resumo**: quando um pedido contém itens de filiais retira diferentes, o maxPedido pode separar o pedido por filial conforme configuração do parâmetro **DESMEMBRAR_PED_FILIAL_RETIRA**.

**Fonte canônica**: manter o passo a passo, parâmetro e restrições de autorização em `05-PEDIDOS-E-VENDAS.md`, seção **Desmembramento de Pedido por Filial Retira**.
...
Esta seção foi mantida apenas como ponte para evitar duplicidade. As consultas SQL de suporte ficam no documento canônico `05-SQL-BANCO-E-INTEGRACAO.md`, incluindo:

- versão do banco na nuvem;
- tabelas com erro de integração/status `-1`;
- produtos positivados por vendedor;
- região e praça de cliente;
- estoque disponível e cotas;
- JSON de pedidos;
- serviço de agendamento Oracle;
- devoluções do resumo de vendas;
- integridade de produto em tabelas relacionadas.
...
Esta seção foi mantida apenas como índice para evitar duplicidade com o catálogo canônico.

**Fonte canônica**: `04-PARAMETROS-E-CONFIGURACAO.md`, que concentra descrição, tipo, escopo e tabela dos parâmetros do maxPedido/Winthor.

**Grupos cobertos no catálogo canônico**:
- GPS, localização, check-in/check-out e roteiro.
- Bloqueio de clientes, inadimplência, limite e controle de envio.
- Conta corrente, estoque, produtos, pedido, negociação e filial retira.
- Campanhas, descontos, exibição, relatórios, cadastro de clientes, filtros e sincronização.
- Parâmetros específicos Winthor.
