...
**Resumo**: check-in/check-out, GPS e cerca eletrônica podem bloquear ou condicionar a confecção de pedidos conforme parâmetros da Central de Configurações.

**Fonte canônica**: detalhes dos parâmetros ficam em `04-PARAMETROS-E-CONFIGURACAO.md`; consultas SQL relacionadas ficam em `08-SCRIPTS-UTILITARIOS.md`.
...
**Resumo**: permite que o pedido seja feito em uma filial, mas a retirada do estoque ocorra em outra. Quando houver itens com filiais retira diferentes, o processo pode gerar desmembramento por filial se configurado.

**Fonte canônica**: regras de pedido e desmembramento ficam em `05-PEDIDOS-E-VENDAS.md`; parâmetros ficam em `04-PARAMETROS-E-CONFIGURACAO.md`.
...
Consulta mantida no documento canônico de SQL: `05-SQL-BANCO-E-INTEGRACAO.md`, seção **Locks na PCMXSINTEGRACAO** e demais consultas de bloqueio.
...
**Resumo**: endpoints de integração cobrem entrada, saída/retorno, status de pedidos, clientes e endereços, com payloads e regras de obrigatoriedade por API.

**Fonte canônica**: manter métodos, direção, campos, payloads, erros e regras de negócio em `01-LAYOUT-INTEGRACAO.md`.
...
Consulta de acompanhamento mantida em `05-SQL-BANCO-E-INTEGRACAO.md`, seção **Registros pendentes de integracao**.
...
Consultas de locks e comandos de apoio ficam no documento canônico `05-SQL-BANCO-E-INTEGRACAO.md`. Use `08-SCRIPTS-UTILITARIOS.md` quando a necessidade for um checklist operacional de banco/ambiente.
...
**Resumo**: quando habilitado, o maxPedido pode separar o pedido por filial retira dos itens. Validações e restrições de autorização devem ser consultadas no documento de pedidos.

**Fonte canônica**: `05-PEDIDOS-E-VENDAS.md`, seção **Desmembramento de Pedido por Filial Retira**.
