...
## Metadados
- Tipo detectado: `md`
- Gerado em: `2026-02-12T17:07:28.586673+00:00`
- Fatos extraidos (LangExtract): `63`
- Caracteres extraidos da fonte: `45851`

## Conhecimento Extraido
[Heuristica] Fatos estruturados sugeridos (fallback local):
- [regra] - Ingestao recomendada somente do documento unificado:
- [regra] - "Quais configuracoes sao criticas para bloqueios, estoque e financeiro?"
- [regra] | Bloquear item sem estoque | `BLOQUEAR_INSERIR_ITEM_SEM_ESTOQUE`, `BLOQUEAR_VENDA_ACIMA_DO_ESTOQUE` | Impede inserir/vender sem disponibilidade |
- [regra] | Cliente bloqueado | `BLOQUEAR_CONFECCAO_PEDIDO_CLIENTE_BLOQ`, `BLOQUEAR_CONFECCAO_PEDIDO_CLIENTE_PRINC_BLOQ`, `ACEITAVENDAAVISTACLIBLOQ`, `CON_ACEITAVENDABLOQ` | Permite ou bloqueia venda conforme politica |
- [regra] | Limite de credito | `BLOQ_ENVIO_PEDIDO_ACIMA_LIMITE`, `ATUALIZAR_LIMCRED_CLIENTE_POS_PEDIDO`, `EXIBIR_ALERTA_CREDITO_CLIENTE` | Controla bloqueio e exibicao de limite |
- [regra] | Visita avulsa | `OBRIGA_CHECKIN_VISITA_AVULSA` + permissao de usuario | Define obrigatoriedade de check-in em visita avulsa |
- [regra] - `GPS_UPDATE_COORDENADAS_SOMENTE_SE_NAO_PREENCHIDO`
- [regra] #### Parametros de pedidos e bloqueios
- [regra] - `BLOQUEIA_ENVIO_ORCAMENTO_ERP`
- [regra] #### Parametros de credito, bloqueio e financeiro
- [regra] - `BLOQUEAR_CONFECCAO_PEDIDO_CLIENTE_BLOQ`
- [regra] - `BLOQUEAR_CONFECCAO_PEDIDO_CLIENTE_PRINC_BLOQ`
- [regra] - `BLOQUEIA_PEDIDO_CLIENTE_INADIMPLENTE`
- [regra] - `BLOQUEIA_PEDIDO_CLIENTE_SEMLIMITE`
- [regra] - `ACEITAR_DIGITAR_PEDIDO_CLIREDEBLOQUEADO`
- [regra] - `BLOQUEAR_SALVAR_PEDIDO_SEMSALDORCA`
- [regra] - `BLOQUEAR_INSERIR_ITEM_SEM_ESTOQUE`
- [regra] - `BLOQUEAR_VENDA_ACIMA_DO_ESTOQUE`
- [regra] - `EXIBIR_ESTOQUE_BLOQUEADO`
- [regra] - `HABILITA_CLIENTES_BLOQUEIO_DEFINITIVO`
- [regra] - `BLOQUEIA_PRACA_PADRAO`
- [regra] - `EXCLUIR_SOMENTE_LIDAS`
- [regra] | 6 | PedidoBloqueadoEnvioErp |
- [regra] | 7 | PedidoBloqueadoCancelado |
- [regra] | 2 | Pedido bloqueado na nuvem |
- [regra] Regra obrigatoria:
- [regra] 1. `MXSINTEGRACAOPEDIDO.DTABERTURAPEDPALM` deve existir no envio.
- [regra] 2. ERP deve devolver o mesmo `DTABERTURAPEDPALM` em `MXSHISTORICOPEDC`.
- [regra] 1. `BLOQUEIA_PEDIDO_CLIENTE_SEMLIMITE` (pedido pode ficar preso por limite).
- [regra] | FPU | Faixa unica de pedido | Regras/faixas unicas, podendo exigir produtos obrigatorios |
- [regra] 2. Edicao/exclusao deve ser feita na aba de campanhas.
- [regra] 3. Em FPU, todas as validacoes necessarias devem estar atendidas para incluir itens.
- [regra] Reune o fluxo de roteiro, visitas avulsas, consultas operacionais do app e bloqueio por ferias.
- [campo_parametro] 2. Em parametros, habilitar `HABILITA_FERIAS_VENDEDOR`. (tokens=HABILITA_FERIAS_VENDEDOR)
- [campo_parametro] ORDER BY DATA_STATUS DESC; (tokens=DATA_STATUS)
- [regra] - SESSOES BLOQUEADAS (LOCKED)
- [regra] - VERIFICAR SE O CLIENTE ESTA BLOQUEADO
- [campo_parametro] UPDATE TABELA_ALVO SET ATUALIZID = '-9999999999' WHERE CONDICAO; (tokens=TABELA_ALVO)
- [campo_parametro] SELECT SID, SERIAL#, STATUS, USERNAME, OSUSER, PROGRAM, BLOCKING_SESSION, EVENT (tokens=BLOCKING_SESSION)
- [regra] #### Plano de pagamento e bloqueios
- [regra] SELECT * FROM MXSPARAMFILIAL WHERE NOME LIKE '%FIL_BLOQUEARPEDIDOSABAIXOVLMINIMO%';
- [regra] - Qualquer referencia a senha/token deve usar placeholder.
- [campo_parametro] SELECT * FROM ALL_TABLES WHERE TABLE_NAME IN ('MXSINTEGRACAOPEDIDO','MXSPARAMETRO','PCPEDC','PCCLIENT'); (tokens=ALL_TABLES, TABLE_NAME)
- [regra] | Pedido preso status 0/5 | Extrator/job/fila ou bloqueio de credito | Checar status, jobs, limite cliente | Normalizar causa e reenfileirar fluxo |
- [regra] | Cliente bloqueado com comportamento errado | Parametros conflitantes | Revisar matriz de bloqueio | Uniformizar politica em maxPedido + ERP |
- [campo_parametro] | Filial retira nao desmembra | Parametro ou fluxo em autorizacao | Checar `DESMEMBRAR_PED_FILIAL_RETIRA` | Ajustar parametro e fluxo operacional | (tokens=DESMEMBRAR_PED_FILIAL_RETIRA)
- [campo_parametro] | Boleto nao abre/imprime | Linha digitavel indisponivel | Validar `EXIBE_LINHA_DIGITAVEL` e campos de titulo | Ajustar parametros e dados de titulo | (tokens=EXIBE_LINHA_DIGITAVEL)
- [campo_parametro] | Roteiro nao aparece | Sem roteiro no ERP ou parametro desabilitado | Checar `PCROTACLI` e `HABILITA_CADASTRO_ROTA_CLIENTE` | Corrigir roteiro/carteira e sincronizar | (tokens=HABILITA_CADASTRO_ROTA_CLIENTE)
- [campo_parametro] | Visita avulsa nao cria | Sem permissao ou check-in bloqueando | Validar permissao e `OBRIGA_CHECKIN_VISITA_AVULSA` | Ajustar perfil/parametro | (tokens=OBRIGA_CHECKIN_VISITA_AVULSA)
- [regra] | Visita avulsa nao cria | Sem permissao ou check-in bloqueando | Validar permissao e `OBRIGA_CHECKIN_VISITA_AVULSA` | Ajustar perfil/parametro |
- [campo_parametro] | Mapa de oportunidades vazio | Filtros restritivos ou parametro off | Checar `HABILITA_MAPA_OPORTUNIDADE` e perfil | Ajustar raio/CNAE/perfil | (tokens=HABILITA_MAPA_OPORTUNIDADE)
- [campo_parametro] | Ferias nao bloqueia acesso | Permissao/parametro/cadastro incompleto | Checar permissao "Ferias" + `HABILITA_FERIAS_VENDEDOR` | Ajustar cadastro e periodo | (tokens=HABILITA_FERIAS_VENDEDOR)
- [regra] | Ferias nao bloqueia acesso | Permissao/parametro/cadastro incompleto | Checar permissao "Ferias" + `HABILITA_FERIAS_VENDEDOR` | Ajustar cadastro e periodo |
- [regra] - Bloqueio de bonificacao sem saldo CC:
- [campo_parametro] - `EXIBIR_PRECO_UNIT_EMB`, `EXIBIR_CAMPOS_ESPELHO_POR_EMBALAGEM`. (tokens=EXIBIR_PRECO_UNIT_EMB, EXIBIR_CAMPOS_ESPELHO_POR_EMBALAGEM)
- [regra] - "Quais testes de recuperacao RAG devem ser executados?"
- [campo_parametro] | 2. Parametros e configuracao | 01-PARAMETROS-E-CONFIGURACAO, 02, 03, 04, CONTROLE_FERIAS, GLOSSARIO | (tokens=CONTROLE_FERIAS)
- [campo_parametro] | 8. Tabelas por dominio | CARGATOTAL, SERVICE-DESK-PROCESSOS, SERVICE_DESK_MAXIMA_ORGANIZADO | (tokens=SERVICE_DESK_MAXIMA_ORGANIZADO)
- [regra] - Parametrizacao de bloqueio de cliente: consolidada matriz com combinacoes de permitir/bloquear e alinhamento ERP.
- [regra] 1. Pergunta: "Qual parametro bloqueia item sem estoque?"
- [regra] Resultado esperado: `BLOQUEAR_INSERIR_ITEM_SEM_ESTOQUE` com contexto e categoria.
- [regra] 8. Pergunta: "Como diagnosticar cliente bloqueado no pedido?"
- [regra] Resultado esperado: matriz de parametros de bloqueio + validacao ERP.
